...
import org.titan.argus.client.entities.dependency.JarDependencies;
import org.titan.argus.client.entities.dependency.PomDependency;
import org.titan.argus.client.entities.dependency.PomInfo;
import org.titan.argus.client.test.TestArgusPomPath;
...
public class Analyzer {
    ...
    private static List<PomInfo> ALL_JAR_POM_INFO;

    private static final String META_PATH = "META-INF";

    public static List<PomInfo> getAllJarPomInfo() {
    	if (ALL_JAR_POM_INFO == null) {
    		ALL_JAR_POM_INFO = analysisPom();
		}
    	return ALL_JAR_POM_INFO;
	}

    private static List<PomInfo> analysisPom() {
        List<PomInfo> pomInfos = new ArrayList<>();
		URL resource = TestArgusPomPath.class.getClassLoader().getResource("");
		System.err.println(resource);
       	try {
			Enumeration<URL> urls = Analyzer.class.getClassLoader().getResources(META_PATH);
    ...
    private static void readPomInfo(String location, List<PomInfo> pomInfos) {
        ...
        try {
            ...
            while ((zipEntry = zip.getNextEntry()) != null) {
                ...
                if (zipEntryPath.startsWith(META_PATH + "/maven") && zipEntryPath.endsWith("pom.xml")) {
                    logger.debug("zipEntryPath: " + zipEntryPath);
                    pomInfo = readPom(zip);
                    break;
                }

                if (zipEntryPath.equals(META_PATH + "/MANIFEST.MF")) {
                    properties.load(zip);
                }
            }
    ...
    private static void setSpringCloudDeps(Optional<PomInfo> optionalPomInfo, //
        ...
        if (optionalPomInfo.isPresent()) {
            ...
            if (StringUtils.isNotEmpty(sccommonsVersion)) {
                jarDependencies.setSpringCloudVersion("");
            }
        }
    }

	public static void main(String[] args) throws Exception {
		List<PomInfo> allJarPomInfo = Analyzer.getAllJarPomInfo();
		JarDependencies allPomInfo = getAllPomInfo();
		String springCloudVersion = allPomInfo.getSpringCloudVersion();
	}
}
